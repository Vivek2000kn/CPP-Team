...
#include"cpp_chess_v8.hpp"

using namespace std;

const int boardSize = 5;                    //board size
...
int main()
{
    ...
    while (1)
    {
        ...
        if(STATUS_OK == checker(UserIP) && STATUS_OK == repeaterChecker(UserIP) && STATUS_OK == obj2.ghostChecker(UserIP))
        {
            if(UserIP == (xIndex*10 + yIndex))              //for repeating entry after another;
                display();
            else if(STATUS_OK == checker(UserIP))           //checks if user input is with board limit or not;
            {
                endStatus = obj1.setIndexData(UserIP, obj2);
            }
            else
                cout << "Invalid index\n";
        ...
        else if(STATUS_ERR == repeaterChecker(UserIP))      //checks if user input is valid or not!;
        {
