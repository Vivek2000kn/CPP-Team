...
/*-----------------BL START --------------------*/
    moveDefaulter();
    ghostMoves(0, 0, BOARD_SIZE);
    recoder(0);
    //moveRecord[rIndex] = uip;
    //rIndex++;
/*    status = blCode(0, 0, BOARD_SIZE);
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (ghostChecker(gridY, gridX) == STATUS_OK  && repeaterChecker((gridY * 10) + gridX, BOARD_SIZE) == STATUS_OK)
                    {
                        ghostMoves(gridY, gridX, BOARD_SIZE);
