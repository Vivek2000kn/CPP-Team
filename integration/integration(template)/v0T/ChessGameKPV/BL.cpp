...
{
    GAMESTATUS winStatus = stopper(bo_size);
...
{
    RET_STATUS ss = STATUS_OK;
//    RET_STATUS rr = checker((x * 10) + y, boardSize);
//   cout << "r " << rr<<endl;
    if (STATUS_OK == checker(((x * 10) + y), boardSize)) // CLRD;
    {
        //RET_STATUS s;
        ghostMoves(x, y, boardSize);
        //cout << s<<endl;
        //return s;
        //ghostChecker((x * 10) + y);
        if (flag == 1)
        {
            return ghostChecker(x, y);
        }
        else
        {
            flag = 1;
